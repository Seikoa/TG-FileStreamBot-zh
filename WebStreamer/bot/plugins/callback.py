...
deldbtnmsg=["您已经删除了链接", "您无法撤销此操作", "您可以重新发送文件以生成新链接", "链接已死", "这只是一个按钮，显示您的链接已被删除"]
    ...
    if update.data == "home":
        ...
        if usr_cmd[0] == "msgdelconf2":
            ...
            if get_media_file_unique_id(get_msg) == usr_cmd[2]:
                ...
                await update.answer("对不起你的文件不在服务器上", show_alert=True)
            else:
                await update.answer("消息ID和文件唯一ID不匹配", show_alert=True)
